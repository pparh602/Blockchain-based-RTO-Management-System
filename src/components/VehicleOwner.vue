<template>
  <div class="component-container">
    <h3 v-if="!isLoggedIn">VehicleOwner</h3>
    <h3 v-else-if="isLoggedIn">
      Hi
      <span v-for="Rto in Rtos" v-if="Rto.id == jwt.oid">
        <span v-for="user in Rto.users" v-if="user.uid == jwt.uid">
          {{ user.name }}
        </span>
      </span>
    </h3>
      ...
      <div class="component-inner-container reactive-list" v-if="!isLogin">
        <center>
          <span v-for="Rto in Rtos">
            ...
            <span v-for="user in Rto.users">
              <button
                type="button"
                class="btn btn-danger"
                style="display: block; margin-bottom: 5px"
                v-for="role in roles"
                v-if="role.id == user.roles[0] && role.name == 'VehicleOwner'"
              >
    ...
    <div v-if="isLoggedIn">
      <div class="top-div">
        ...
        <div class="component-inner-container" v-if="currentTab == 'post-doc'">
          <div class="form-row">
            <div class="col">
              <input
                type="text"
                v-model="vehicleowner.postdocname"
                class="form-control"
            ...
            <div class="col">
              <input
                type="text"
                v-model="vehicleowner.postdoccontent"
                class="form-control"
                placeholder="Data"
              />
            </div>
        ...
        <div class="component-inner-container" v-if="currentTab == 'get-doc'">
          <div class="form-row">
            <div class="col">
              <select class="form-control" v-model="vehicleowner.getdocid">
        ...
        <!-- GET ACCESS LOG -->
        <div
          class="component-inner-container"
          v-if="currentTab == 'get-access-log'"
        >
          <div class="form-row">
            <div class="col">
              <select class="form-control" v-model="vehicleowner.getlogdocid">
                <option value="" selected disabled>Select Document</option>
                <option v-for="doc in docListForUser">
                  {{ doc.name }}
                </option>
              ...
              <tbody>
                <tr>
                  ...
                  >
                    <input
                      type="text"
                      v-model="vehicleowner.acladdemailid"
                      class="form-control"
                      name="emailid"
                      id="emailid"
                  ...
                  <td>
                    <a class="add" title="Add" @click="addAccess"
                      ><Octicon :icon="plus"></Octicon
                    ></a>
                  </td>
                </tr>
      ...
      <div
        class="component-shell-container"
        v-if="currentTab != 'acl' && currentTab != 'get-access-log'"
      >
...
<script>
import Api from "@/apis/VehicleOwnerApi";
...
export default {
  ...
  methods: {
    ...
    handleFileUpload() {
      this.vehicleowner.postdocfile = this.$refs.file.files[0];
    },

    async postDocJson() {
      var docName = this.vehicleowner.postdocname;
      var docContent = this.vehicleowner.postdoccontent;
      ...
      if (docName && docContent) {
        ...
        this.getPostDocStatus(
          apiResponse.data.response.correlationId,
          docName,
          0
        );
      }
    ...
    async getPostDocStatus(corrId, docName, attempts) {
      if (corrId) {
        ...
        if (apiResponse.data[corrId].transactionStatus != "initiated") {
          ...
          var userName = null;
          var userEmail = null;
          for (var rto of this.rtos) {
            if (rto.id == this.jwt.oid) {
              for (var user of rto.users) {
          ...
          RedisApi.postDocToUserMapping(
            Object.keys(apiResponse.data[corrId].documentStatus)[0],
            [
              {
                id: this.jwt.uid,
                name: userName,
    ...
    async getDoc() {
      var docName = this.vehicleowner.getdocid;
    ...
    async getAccessLog() {
      var docName = this.vehicleowner.getlogdocid;
    ...
    async getDocACL() {
      var docName = this.vehicleowner.acldocid;
    ...
    async addAccess() {
      var emailId = this.vehicleowner.acladdemailid;

      for (var user of this.userListForDoc) {
        if (user.email == emailId) {
          console.log("User has permissions...");
          this.vehicleowner.acladdemailid = "";
          return;
        }
      }
      ...
      if (userId && vehicleOwnerId && vehicleOwnerName && emailId) {
    ...
    async postDoc() {
      var docName = this.vehicleowner.postdocname;
      var docFile = this.vehicleowner.postdocfile;
